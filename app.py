...
UPLOAD_FOLDER = os.path.join(BASE_DIR, 'static/uploads/payment_proofs')
    ...
    if session["role"] == "admin":
        ...
        for loan in my_loans:
            ...
            if loan["status"] == 'approved' and loan["repayment_status"] == 'open':
                ...
                # Get payment type (default to 'monthly' for old loans that don't have this column)
                try:
...
def request_loan():
    # Restricted: Only admin can create loans now
    if session.get("role") != "admin": 
        flash("Only admins can create loans. Please contact an administrator.")
        return redirect(url_for("dashboard"))
...
def admin_loans():
    ...
    for loan in active_loans_raw:
        ...
        if payment_type == 'lump_sum':
            # For lump sum, show full amount + total interest as the payment due
            if paid_count == 0:
                total_interest = int((loan['amount'] * loan['interest_rate_percent'] * loan['total_months']) / 100)
            ...
            # For monthly EMI, use reducing balance calculation
            emi_calc = calculate_dynamic_emi(loan["amount"], loan["total_months"], loan["interest_rate_percent"], next_month)
            if emi_calc:
    ...
    all_members = db.execute("SELECT * FROM members WHERE role='member' ORDER BY name").fetchall()
...
@app.route("/admin/create_loan", methods=["POST"])
def create_loan():
    if session.get("role") != "admin": return redirect(url_for("login"))
    ...
    amount = int(request.form["amount"])
    months = int(request.form["months"])
    interest_rate = float(request.form.get("interest_rate", 1))
    payment_type = request.form.get("payment_type", "monthly")
    
    # Calculate loan components based on payment type
    if payment_type == "lump_sum":
        # Lump sum: Pay total amount + (interest_rate * months * amount / 100) at the end
        total_interest = int((amount * interest_rate * months) / 100)
        interest_portion = total_interest  # Total interest for the entire period
        principal_portion = amount  # Full principal
        emi_amount = amount + total_interest  # Single payment at the end
    else:
        # Monthly EMI: Reducing balance method
        interest_portion = int((amount * interest_rate) / 100)
        principal_portion = int(amount / months)
        emi_amount = principal_portion + interest_portion
...
def loan_tracking():
    ...
    for loan in loans:
        ...
        # Get payment type (default to 'monthly' for old loans)
        try:
            payment_type = loan['payment_type'] if loan['payment_type'] else 'monthly'
        except (KeyError, IndexError):
            payment_type = 'monthly'
        ...
        if loan["repayment_status"] == 'open':
            ...
            if payment_type == 'lump_sum':
                # Lump sum: Show full payment amount
                if paid_count == 0:
                    total_interest = int((loan['amount'] * loan['interest_rate_percent'] * loan['total_months']) / 100)
                ...
                # Monthly EMI: Use reducing balance
                emi_calc = calculate_dynamic_emi(loan["amount"], loan["total_months"], loan["interest_rate_percent"], next_month)
                if emi_calc:
...
def submit_payment_proof():
    # Restricted: Only admin can submit payment proofs now
    if session.get("role") != "admin":
        flash("Only admins can submit payment proofs. Please contact an administrator.")
        return redirect(url_for("dashboard"))
...
def delete_interest(id):
    ...
    payment = db.execute("SELECT * FROM interest_payments WHERE id=?", (id,)).fetchone()
    ...
    if not loan:
        flash("❌ Associated loan not found.")
        return redirect(url_for("admin_manage_payments"))
    ...
    # Get all remaining payments for this loan ordered by date
    all_payments = db.execute(
        "SELECT * FROM interest_payments WHERE loan_id=? ORDER BY paid_date ASC",
        (loan_id,)
    ).fetchall()
    ...
    # Recalculate balance with remaining payments
    ...
    # Update loan balance and reopen if it was closed
    if new_balance > 0:
        db.execute(
            "UPDATE loans SET remaining_balance=?, repayment_status='open', closed_time=NULL WHERE loan_id=?",
            (new_balance, loan_id)
        )
    else:
        # Keep as closed if balance is still 0
        db.execute(
    ...
    db.commit()
    flash(f"✅ Payment deleted successfully. Loan balance updated to ₹{new_balance:.2f}")
    return redirect(url_for("admin_manage_payments"))

@app.route("/admin/delete_loan/<int:loan_id>", methods=["POST"])
def delete_loan(loan_id):
    if session.get("role") != "admin": return redirect(url_for("login"))
    db = get_db()
    
    # Delete all associated interest payments first
    db.execute("DELETE FROM interest_payments WHERE loan_id=?", (loan_id,))
    
    # Delete all associated payment proofs
    db.execute("DELETE FROM payment_proofs WHERE loan_id=?", (loan_id,))
    
    # Delete the loan
    db.execute("DELETE FROM loans WHERE loan_id=?", (loan_id,))
    
    db.commit()
    flash("✅ Loan and all associated payments deleted successfully.")
    return redirect(url_for("admin_loans"))

@app.route("/admin/loan_payment_history/<int:loan_id>")
def loan_payment_history(loan_id):
    ...
    db = get_db()
    payments = db.execute(
        "SELECT * FROM interest_payments WHERE loan_id=? ORDER BY paid_date DESC",
        (loan_id,)
    ).fetchall()
...
@app.route("/admin/undo_payment/<int:payment_id>", methods=["POST"])
def undo_payment(payment_id):
    if session.get("role") != "admin": return redirect(url_for("login"))
    db = get_db()
    
    # Get the payment details
    payment = db.execute("SELECT * FROM interest_payments WHERE id=?", (payment_id,)).fetchone()
    ...
    # Calculate the principal portion that was paid
    current_balance = loan['remaining_balance']
    interest_rate = loan['interest_rate_percent']
    
    # Recalculate what the balance was before this payment
    # The payment reduced the balance by (payment_amount - interest_component)
    # We need to reverse this
    # Note: This is an approximation since interest was calculated on old balance
    
    # Get all payments for this loan ordered by date to recalculate properly
    all_payments = db.execute(
        "SELECT * FROM interest_payments WHERE loan_id=? ORDER BY paid_date ASC",
        (loan_id,)
    ).fetchall()
    ...
    # Recalculate balance excluding this payment
    for pmt in all_payments:
        if pmt['id'] == payment_id:
            continue  # Skip the payment we're undoing
    ...
    # Delete the payment
    db.execute("DELETE FROM interest_payments WHERE id=?", (payment_id,))
    ...
    db.commit()
    flash(f"✅ Payment undone successfully. Loan balance restored to ₹{new_balance:.2f}")
    return redirect(url_for("admin_loans"))
